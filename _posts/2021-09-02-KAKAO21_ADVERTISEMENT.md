---
title: "[프로그래머스] 2021 카카오 개발자 블라인드 채용 > 광고 삽입 (72414)(Kotlin)"
excerpt: 카카오TV에서 유명한 크리에이터로 활동 중인 `죠르디`는 환경 단체로부터 자신의 가장 인기있는 동영상에 지구온난화의 심각성을 알리기 위한 공익광고를 넣어 달라는 요청을 받았습니다. 평소에 환경 문제에 관심을 가지고 있던 "죠르디"는 요청을 받아들였고 광고효과를 높이기 위해 시청자들이 가장 많이 보는 구간에 공익광고를 넣으려고 합니다. "죠르디"는 시청자들이 해당 동영상의 어떤 구간을 재생했는 지 알 수 있는 재생구간 기록을 구했고, 해당 기록을 바탕으로 공익광고가 삽입될 최적의 위치를 고를 수 있었습니다.
categories:
- programmers
tags: 카카오, 코딩테스트
last_modified_at: 2021-09-02T11:00:00+0900
---

**[원본 문제 보기](https://programmers.co.kr/learn/courses/30/lessons/72414?language=kotlin){:target="_blank"}**
...
- 그림의 검은색 선들은 각 시청자들이 "죠르디"의 동영상을 재생한 구간의 위치를 표시하고 있습니다.
  ...
  - 위 그림에서, 3번 재생 기록은 `00시 25분 50초` 부터 `00시 48분 29초` 까지 총 `00시간 22분 39초` 동안 죠르디의 동영상을 재생했습니다. [1](https://programmers.co.kr/learn/courses/30/lessons/72414?language=kotlin#fn1)
...
- 동영상 재생시간과 공익광고 재생시간이 같으므로, 삽입할 수 있는 위치는 맨 처음(`00:00:00`)이 유일합니다.
  ...
  1. `동영상 재생시간 = 재생이 종료된 시각 - 재생이 시작된 시각`(예를 들어, `00시 00분 01초`부터 `00시 00분 10초`까지 동영상이 재생되었다면, 동영상 재생시간은 `9초` 입니다.) [↩](https://programmers.co.kr/learn/courses/30/lessons/72414?language=kotlin#fnref1)
...
```kotlin
class Solution {
    data class TimeStamp(val startTime: Int, val endTime: Int, val durations: Int): Comparable<TimeStamp> {
    override fun compareTo(other: TimeStamp): Int {
        return if (this.durations == other.durations) {
    ...
    fun solution(play_time: String, adv_time: String, logs: Array<String>): String {
        val duration = timeToSec("00:00:00-$play_time")[1]
        val advTime = timeToSec("00:00:00-$adv_time")[1]
        var timeArr = IntArray(duration + 1)
        logs.forEach {
            val (s, e) = timeToSec(it)
            for (i in s..e) {
                timeArr[i] += 1
            }
        }
        ...
        while (true) {
            if (right >= duration - 1) break

            if (sum > max) {
                max = sum
                list.add(TimeStamp(left, right, sum))
            }
            sum -= timeArr[left++]
            sum += timeArr[++right]
        }
        ...
        return if (list.isNotEmpty()) {
            "${secToTime(list.sortedDescending()[0].startTime)}"
        } else {
            "00:00:00"
        }
    }

    fun timeToSec(str: String): IntArray {
        ...
        startTime += time[2]
        startTime += time[1] * 60
        startTime += time[0] * 3600

        endTime += time[5]
        endTime += time[4] * 60
        endTime += time[3] * 3600

        return intArrayOf(startTime, endTime)
    }
